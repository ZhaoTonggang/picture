			...
			if (isPC()) {
				$('#walBox').onepage_scroll({
					...
					beforeMove: function(index) {
						$("#toolBall").attr('href', $(".section").eq(index - 1).attr(
							'data-url'));
						$(".section").eq(index - 1).attr('style', 'background-image: url(' + $(
							".section").eq(index - 1).attr('data-img') + ')');
					},
					afterMove: function(index) {
						$(".section").eq(index).attr('style', 'background-image: url(' + $(
							".section").eq(index).attr('data-img') + ')');
						$(".section").eq(index - 2).attr('style', 'background-image: url(' + $(
							".section").eq(index - 2).attr('data-img') + ')');
				...
				$(".section").eq(0).attr('style', 'background-image: url(' + $(".section").eq(0).attr(
					'data-img') + ')');
			}
		}
	});
	...
	$.ajax({
		...
		success: function(jsonData) {
			...
			for (var i = 0; i < jsonData.data.length; i++) {
				...
				newHtml += '<a class="dropdown-item" href="javascript:void(0)" data-id=' + jsonData.data[i]
					.old_id + ' onclick="loadData(' + jsonData.data[i].old_id +
					', true);changeTitle(this)">' + jsonData.data[i].category + '</a>';
			}
